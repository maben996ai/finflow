...
- 先阅读相关代码、文档和测试，再开始修改。
- 遵循 TDD：先写测试，再写实现，每轮实现后运行单元测试确认通过。
- 按功能/文件分步骤开发，每个功能遵循以下顺序：
  1. **上下文确认**：阅读相关代码，明确改动范围和验证方式
  2. **迭代开发**：分多轮实现，每轮完成后运行单元测试
  3. **集成测试**：跑完整测试套件，确认无回归
  4. **真实验证**：用真实数据或真实接口验证功能
  5. **收尾提交**：整理改动，提交并推送
- 未经要求，不修改无关文件，不做大范围重构。
- 默认使用中文输出。
- 修改后必须说明：改了哪些文件、做了什么、如何验证、还有什么风险。
